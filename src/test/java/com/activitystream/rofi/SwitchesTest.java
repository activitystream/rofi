...
public class SwitchesTest {
    ...
    public void order_of_sources_dictates_priorities_of_value() {
        ...
        when(src2.switchValue(anyString())).thenReturn("1");
    ...
    public void first_source_to_have_value_is_used() {
        when(src1.switchValue(anyString())).thenReturn(null);
        when(src2.switchValue(anyString())).thenReturn("0");
        when(src3.switchValue(anyString())).thenReturn("1");
    ...
    public void value_of_1_or_TRUE_turns_switch_on() {
        when(src1.switchValue(anyString())).thenReturn("1");
        when(src1.switchValue(anyString())).thenReturn("TRUE");
        when(src1.switchValue(anyString())).thenReturn("truE");
    ...
    public void value_different_from_1_or_TRUE_turns_switch_off() {
        when(src1.switchValue(anyString())).thenReturn("2");
        when(src1.switchValue(anyString())).thenReturn("FOR REAL");
        when(src1.switchValue(anyString())).thenReturn("YES");
